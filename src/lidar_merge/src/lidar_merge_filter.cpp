...
public:
    ...
    void applyROIFilter(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max) {
        // X축 필터링
    ...
    // 각 LiDAR 토픽에 대한 콜백 함수와 ROI 설정
    void pointCloudCallback_r(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, -50, 50.0, -50, -2.32, -3.4, 5.0);  // 예시 ROI 값
        mergeClouds();
    }

    void pointCloudCallback_fr(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, 0.0, 50.0, 0, 6.32, -0.643, 5.0);  // 예시 ROI 값
        mergeClouds();
    }

    void pointCloudCallback_fl(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, -50.0, 0.0, 0, 6.32, -0.643, 5.0);  // 예시 ROI 값
        mergeClouds();
    }

    void pointCloudCallback_rr(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, -7.0, 7.0, -2.32, 0, -0.643, 5.0);  // 예시 ROI 값
        mergeClouds();
    }

    void pointCloudCallback_rl(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, -50.0, 0.0, -2.32, 0, -0.643, 5.0);  // 예시 ROI 값
        mergeClouds();
    }

    void pointCloudCallback_f(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        copyPointCloud(msg);
        applyROIFilter(transformedCloud, -50, 50.0, 6.32, 100.0, -1.043, 5.0);  // 예시 ROI 값
        mergeClouds();
    }
    ...
    Process() : Node("lidar_fusion") {
        ...
        subpointCloudTopic_r = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_r, 10, std::bind(&Process::pointCloudCallback_r, this, std::placeholders::_1));
        subpointCloudTopic_fr = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_fr, 10, std::bind(&Process::pointCloudCallback_fr, this, std::placeholders::_1));
        subpointCloudTopic_fl = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_fl, 10, std::bind(&Process::pointCloudCallback_fl, this, std::placeholders::_1));
        subpointCloudTopic_rr = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_rr, 10, std::bind(&Process::pointCloudCallback_rr, this, std::placeholders::_1));
        subpointCloudTopic_rl = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_rl, 10, std::bind(&Process::pointCloudCallback_rl, this, std::placeholders::_1));
        subpointCloudTopic_f = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            pointCloudTopic_f, 10, std::bind(&Process::pointCloudCallback_f, this, std::placeholders::_1));
